...
alberghi *inizializza(alberghi *p)
{
    ...
    else
    {
        ...
        p2=p;
             ...
             /*scrivere una funzione parser che prende in ingresso il vettore v e restituisce una variabile di tipo alberghi
             if(p2==NULL){
             p2=(alberghi*)malloc(sizeof(alberghi));
             settare i campi del nodo puntati da p2
             p2->next==NULL;
             }else{
             dummy=(alberghi*)malloc(sizeof(alberghi));
             settare i campi del nodo puntati da p2
             dummy->next=p2;
             p2=dummy;
             }*/
...
alberghi *stampa_s_s(alberghi *p)
{
    ...
    while(p!=NULL)
    {
        if(p2->albergo.servizio==servizio && p2->albergo.stelle==stelle)
        {
            printf("%s",p->albergo);
            p=p->next;
            p2=p;
